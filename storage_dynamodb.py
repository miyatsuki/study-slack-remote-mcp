...
class DynamoDBTokenStorage(TokenStorageInterface):
    ...
    def __init__(self, table_name: str = None, region: str = None):
        if not BOTO3_AVAILABLE:
            raise ImportError("boto3 is required for DynamoDB storage. Install with: pip install boto3")
        
        self.table_name = table_name or os.getenv("DYNAMODB_TABLE_NAME", "slack-mcp-tokens")
    ...
    def save_token(self, client_id: str, token: str, expires_in_seconds: Optional[int] = None) -> bool:
        ...
        try:
            ...
            print(f"✅ トークンをDynamoDBに保存しました (クライアント: {client_id[:8]}...)")
            return True
    ...
    def load_token(self, client_id: str) -> Optional[str]:
        ...
        try:
            ...
            if self._is_token_expired(item):
                print(f"⚠️ DynamoDB保存トークンが期限切れです (クライアント: {client_id[:8]}...)")
                # Delete expired token
                self.table.delete_item(Key={'client_id': client_id})
                return None
            
            print(f"✅ DynamoDB保存トークンを使用します (クライアント: {client_id[:8]}...)")
    ...
    def cleanup_expired_tokens(self):
        ...
        try:
            ...
            if expired_count > 0:
                print(f"🧹 DynamoDBから期限切れトークン {expired_count} 件を削除しました")
